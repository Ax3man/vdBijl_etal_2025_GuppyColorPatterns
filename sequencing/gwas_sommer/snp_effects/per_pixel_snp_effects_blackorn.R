...
gwas <- prep_sommer_gwas_table(
  str_glue('mel_{c(1, 3, 5:8)}'), name = trait, pval_column = pval_column
) |>
  filter(
    str_starts(chr, 'NC'),
...
# Load the phenotypes ------------------------------------------------------------------------------
img_files <- list.files('data/melanic_coloration_warped_v2/', recursive = TRUE, full.names = TRUE) |>
...
fitGWAS <- function(
    ...
    outfile = str_glue('sequencing/gwas_sommer/snp_effects/pixel_results/black_ornaments/{reference}_{name}.csv.gz')
) {
  df_geno <- filter(df, fish_id %in% geno_id)

  brms_file <- str_glue('quant_gen/cluster_brms_melanin/posterior_summaries_{1:2}/{name}.rds')
