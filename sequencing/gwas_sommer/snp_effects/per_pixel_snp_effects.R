...
gwas <- prep_sommer_gwas_table(trait, pval_column = pval_column) |>
  filter(
    str_starts(chr, 'NC'),
    p.value.adj < fdr.level
  )

gwas_selected <- reduce_peaks(
  gwas |> filter(!(source %in% c('X', 'Y'))),
  min_dist = min_dist, pval_column = pval_column
) |>
  bind_rows(
    gwas |> filter(source %in% c('X', 'Y')) |> arrange(ref) |>
      group_by(chr == 'NC_024342.1', source) |>
...
fitGWAS <- function(
    ...
    outfile = str_glue('sequencing/gwas_sommer/snp_effects/pixel_results/car_PIE/{reference}_{name}.csv.gz')
) {
